            ...
            scraper.obtenerTasas(new BcvScraper.BcvCallback() {
                ...
                public void onSuccess(BcvScraper.BcvData data) {
                    try {
                        ...
                        v.setTextViewText(R.id.widgetDolarRate, String.format("%.2f", data.dolar));
                        v.setTextViewText(R.id.widgetEuroRate, String.format("%.2f", data.euro));
                        SimpleDateFormat sdf = new SimpleDateFormat("hh:mm a", Locale.getDefault());
                        v.setTextViewText(R.id.widgetLastUpdate, sdf.format(new Date()));
                ...
                public void onError(String error, BcvScraper.BcvData cachedData) {
                    try {
                        RemoteViews v = buildViews(appContext);
                        if (cachedData != null) {
                            v.setTextViewText(R.id.widgetDolarRate, String.format("%.2f", cachedData.dolar));
                            v.setTextViewText(R.id.widgetEuroRate, String.format("%.2f", cachedData.euro));
                            v.setTextViewText(R.id.widgetLastUpdate, "Caché");
                        } else {
                            v.setTextViewText(R.id.widgetDolarRate, "---");
                            v.setTextViewText(R.id.widgetEuroRate, "---");
                            v.setTextViewText(R.id.widgetLastUpdate, "Sin datos");
                        }
                        mgr.updateAppWidget(widgetId, v);
                    } catch (Exception e) {
